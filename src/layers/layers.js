...
class Composition {
  ...
  updatePixelOnLayers(x, y, layerIndicesArray, new_r, new_g, new_b, new_a) {
    for (let ii = 0; ii < layerIndicesArray.length; ii++) {
      ...
      if (laynum >= 0 && laynum < this.layers.length) {
        ...
        window.alert(
          "Your array indices are out of bounds!  See layers.js file for reference.",
        );
      }
    }
  }
  ...
  getComposite(x = null, y = null) {
    ...
    for (let aa = 0; aa < this.layers[0].width; aa++) {
      for (let bb = 0; bb < this.layers[0].height; bb++) {
        transparency_grid.pixelData[aa][bb]["r"] = this.layers[0].pixelData[aa][
          bb
        ]["r"];
        transparency_grid.pixelData[aa][bb]["g"] = this.layers[0].pixelData[aa][
          bb
        ]["g"];
        transparency_grid.pixelData[aa][bb]["b"] = this.layers[0].pixelData[aa][
          bb
        ]["b"];
        transparency_grid.pixelData[aa][bb]["a"] = this.layers[0].pixelData[aa][
          bb
        ]["a"];
    ...
    for (let ii = 1; ii < this.layers.length; ii++) {
      ...
      if (blendLayer.shouldDisplay) {
        // used for when layers are solo'd
            ...
            if (a !== 0) {
              ...
              let rr = Math.round(
                a *
                  (255 *
                    blendFunc(
                      baselayer.pixelData[xx][yy]["r"] / 255.0,
                      blendLayer.pixelData[xx][yy]["r"] / 255.0,
                    )) +
                  (1 - a) * blendLayer.pixelData[xx][yy]["r"],
              );
              let gg = Math.round(
                a *
                  (255 *
                    blendFunc(
                      baselayer.pixelData[xx][yy]["g"] / 255.0,
                      blendLayer.pixelData[xx][yy]["g"] / 255.0,
                    )) +
                  (1 - a) * blendLayer.pixelData[xx][yy]["g"],
              );
              let bb = Math.round(
                a *
                  (255 *
                    blendFunc(
                      baselayer.pixelData[xx][yy]["b"] / 255.0,
                      blendLayer.pixelData[xx][yy]["b"] / 255.0,
                    )) +
                  (1 - a) * blendLayer.pixelData[xx][yy]["b"],
              );

              // still have to take the blended color and weight it with this layer's opacity
              baselayer.pixelData[xx][yy]["r"] = Math.round(
                rr * j + baselayer.pixelData[xx][yy]["r"] * (1 - j),
              );
              baselayer.pixelData[xx][yy]["g"] = Math.round(
                gg * j + baselayer.pixelData[xx][yy]["g"] * (1 - j),
              );
              baselayer.pixelData[xx][yy]["b"] = Math.round(
                bb * j + baselayer.pixelData[xx][yy]["b"] * (1 - j),
              );
            } else {
              // since the alpha of the base layer is zero, just return this layer; there's no blending to be done
              baselayer.pixelData[xx][yy]["r"] = Math.round(
                blendLayer.pixelData[xx][yy]["r"] * j +
                  baselayer.pixelData[xx][yy]["r"] * (1 - j),
              );
              baselayer.pixelData[xx][yy]["g"] = Math.round(
                blendLayer.pixelData[xx][yy]["g"] * j +
                  baselayer.pixelData[xx][yy]["g"] * (1 - j),
              );
              baselayer.pixelData[xx][yy]["b"] = Math.round(
                blendLayer.pixelData[xx][yy]["b"] * j +
                  baselayer.pixelData[xx][yy]["b"] * (1 - j),
              );
            }
            baselayer.pixelData[xx][yy]["a"] =
              a + b * blendLayer.opacity - a * b * blendLayer.opacity; // https://stackoverflow.com/a/21492544
          }
        }
      }
    }
  ...
  calculateSoloLayerVisibility() {
    ...
    for (let ii = 1; ii < this.layers.length; ii++) {
      if (temp.length === 0) {
        // no layers are solo'd => all are visible
        this.layers[ii].shouldDisplay = true;
      } else if (temp.includes(ii)) {
        // at least one layer is solo'd, so only display solo'd layers
        this.layers[ii].shouldDisplay = true;
      } // this layer is not solo'd, so don't display it
      else {
