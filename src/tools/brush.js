import { CANVAS_SIZE_X, CANVAS_SIZE_Y } from "../canvasHelpers";
    ...
    for (let curY = y - (radius - 1); curY < y + radius; curY++) {
      ...
      for (let curX = x - (radius - 1); curX < x + radius; curX++) {
        ...
        for (let ii = 0; ii < activeLayers.length; ii++) {
          ...
          mainComp.layers[ind].pixelData[curX][curY].r = Math.trunc(
            mainComp.layers[ind].pixelData[curX][curY].r +
              differenceR * percentTargColor,
          );
          mainComp.layers[ind].pixelData[curX][curY].g = Math.trunc(
            mainComp.layers[ind].pixelData[curX][curY].g +
              differenceG * percentTargColor,
          );
          mainComp.layers[ind].pixelData[curX][curY].b = Math.trunc(
            mainComp.layers[ind].pixelData[curX][curY].b +
              differenceB * percentTargColor,
          );
          mainComp.layers[ind].pixelData[curX][curY].a =
            mainComp.layers[ind].pixelData[curX][curY].a +
            differenceA * percentTargColor;
        }
      }
    }
  }
